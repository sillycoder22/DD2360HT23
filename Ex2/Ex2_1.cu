...
//“sudo /usr/local/cuda-12.2/bin/ncu --metrics sm__warps_active.avg.pct_of_peak_sustained_active ./Ex1 131070” 使用这个指令来测量Achieved Occupancy
...
bool compareMatrices(DataType *mat1, DataType *mat2, int numRows, int numColumns) {
    for (int i = 0; i < numRows; ++i) {
        for (int j = 0; j < numColumns; ++j) {
            if (fabs(mat1[i * numColumns + j] - mat2[i * numColumns + j]) > 1e-5) {
